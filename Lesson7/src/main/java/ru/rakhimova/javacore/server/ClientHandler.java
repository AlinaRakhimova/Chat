...
public class ClientHandler {
        ...
        while (!authorizationOk) { // цикл авторизации
            ...
            try {
                ...
                if (str.startsWith(AUTH)) {
                    ...
                    String nick = chatServer.getAuthService().getNickByLogin(parts[1]);
                    checkNick(nick, o);
                }
            } catch (IOException e) {
                LOGGER.severe(e.getMessage());
    ...
    public void checkNick(String nick, ClientHandler o){
        if (nick != null) {
        ...
        while (true) { // цикл получения сообщений
            ...
            if (str != null) {
                ...
                if (str.startsWith("/w")) {
                    ...
                    chatServer.sendMsgByUser(name, messageSplit[1], messageSplit[2]);
    ...
    class RunnableClassAuthorizationAndRead implements Runnable {
        ...
        RunnableClassAuthorizationAndRead(ClientHandler o) { this.o = o;}
